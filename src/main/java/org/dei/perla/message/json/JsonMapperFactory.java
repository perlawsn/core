...
public class JsonMapperFactory extends AbstractMapperFactory {
	...
	private Class<FpcMessage> createObject(JsonObjectDescriptor desc,
		...
		String className = "org.dei.perla.fpc." + desc.getId()
		...
		try {
			...
			ctx.ctClass.addInterface(ctx.classPool
					.get("org.dei.perla.message.FpcMessage"));
	...
	private CtClass addListFieldClass(JsonValueDescriptor field,
		...
		ctx.validatorCode
				.append("for (int i = 0; i < $0."
						...
						+ "if (!($0."
						+ field.getName()
						+ ".get(i) instanceof org.dei.perla.message.FpcMessage)) { continue; }"
						+ "org.dei.perla.message.FpcMessage m = (org.dei.perla.message.FpcMessage) $0."
						+ field.getName()
						+ ".get(i);\n"
						+ "if (m instanceof org.dei.perla.message.FpcMessage && !m.validate()) {\n\t\t return false; \n\t } \n}");
	...
	private void addStaticValue(JsonValueDescriptor field,
		...
		case TIMESTAMP:
			ctx.constructorCode.append("java.time.format.DateTimeFormatter fmt;"
					...
					+ "$0."
					+ field.getName()
					+ " = org.dei.perla.utils.DateUtils.format(fmt, " + field.getValue() + ");");
			break;
		default:
	...
	private void addTimestampAttributeGetSetCode(JsonValueDescriptor field,
		...
		context.getFieldCode.append("if (\"" + field.getName()
				...
				+ "fmt = java.time.format.DateTimeFormatter.ofPattern(\""
				+ field.getFormat() + "\");"
				+ "return org.dei.perla.utils.DateUtils.parse(fmt, $0."
				+ field.getName() + "); }");
		...
		if (!field.isStatic()) {
			context.setFieldCode
					.append("if (\""
							...
							+ "$0."
							+ field.getName()
							+ " = org.dei.perla.utils.DateUtils.format(fmt, $2); return; }");
		}
